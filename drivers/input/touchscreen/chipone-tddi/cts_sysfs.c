...
static ssize_t cts_panel_name_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(panel_name, S_IRUGO, cts_panel_name_show, NULL);
...
static int cts_fw_class_init(void *_data, bool create)
{
    ...
    static int ts_class_refcnt;
    dev_t devno;

    cts_info("%s touchscreen class files", create ? "Add" : "Remove");

    if (create) {
        /* If class already created, just bump refcnt and return */
	...
	touchscreen_class = class_create(THIS_MODULE, "touchscreen");
        ...
        if (!touchscreen_class || !ts_class_dev || ts_class_refcnt <= 0)
            return -ENODEV;

        /* Only destroy class when last user goes away */
        if (--ts_class_refcnt > 0)
            return 0;
...
int cts_sysfs_add_device(struct device *dev)
{
    ...
    /* Create a unique link under /sys so multiple devices don't clash */
    snprintf(link_name, sizeof(link_name), "%s", dev_name(dev));
    ret = sysfs_create_link(NULL, &dev->kobj, link_name);
    if (ret)
        cts_err("Create sysfs link '%s' error:%d", link_name, ret);
     ...
     * Use unique kobject name to avoid duplicate 'touchscreen' directory.
     */
    snprintf(link_name, sizeof(link_name), "touchscreen-%s", dev_name(dev));
    cts_data->suspend_kobj = kobject_create_and_add(link_name, NULL);
...
void cts_sysfs_remove_device(struct device *dev)
{
    ...
    snprintf(link_name, sizeof(link_name), "%s", dev_name(dev));
    sysfs_remove_link(NULL, link_name);
