...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
...
#ifdef CONFIG_CTS_I2C_HOST
    {
        ...
        snprintf(proc_name, sizeof(proc_name), "%s-%d-%02x",
             OEM_SELFTEST_PROC_FILENAME,
             cts_data->i2c_client->adapter->nr,
             cts_data->i2c_client->addr);

        oem_data->selftest_proc_entry =
            proc_create_data(proc_name,
...
#ifdef CONFIG_CTS_I2C_HOST
    {
        char proc_name[64];
        snprintf(proc_name, sizeof(proc_name), "%s-%d-%02x",
        OEM_FACTORY_TEST_PROC_FILENAME,
        cts_data->i2c_client->adapter->nr,
        cts_data->i2c_client->addr);
...
#ifdef CONFIG_CTS_I2C_HOST
    {
        ...
        cts_data->i2c_client->adapter->nr,
        cts_data->i2c_client->addr);
        oem_data->rawdata_proc_entry = proc_create_data(proc_name,
...
#ifdef CONFIG_CTS_I2C_HOST
    {
        char proc_name[64];
        snprintf(proc_name, sizeof(proc_name), "%s-%d-%02x",
        OEM_MANUAL_PROC_FILENAME,
        cts_data->i2c_client->adapter->nr,
        cts_data->i2c_client->addr);
        oem_data->manual_proc_entry = proc_create_data(proc_name,
...
#ifdef CONFIG_CTS_I2C_HOST
    {
        char proc_name[64];
        snprintf(proc_name, sizeof(proc_name), "%s-%d-%02x",
        OEM_DIFFDATA_PROC_FILENAME,
        cts_data->i2c_client->adapter->nr,
        cts_data->i2c_client->addr);
        oem_data->diffdata_proc_entry = proc_create_data(proc_name,
...
int cts_oem_deinit(struct chipone_ts_data *cts_data)
{
    ...
    if (oem_data->selftest_proc_entry) {
        cts_info("  Remove selftest proc entry");
        proc_remove(oem_data->selftest_proc_entry);
        oem_data->selftest_proc_entry = NULL;
    }

    if (oem_data->limit_entry) {
        cts_info("  Remove limit proc entry");
        proc_remove(oem_data->limit_entry);
        oem_data->limit_entry = NULL;
    }
    if (oem_data->factory_test_entry) {
        cts_info("  Remove factory_test proc entry");
        proc_remove(oem_data->factory_test_entry);
        oem_data->factory_test_entry = NULL;
    }

    if (oem_data->rawdata_proc_entry) {
        cts_info("  Remove rawdata proc entry");
        proc_remove(oem_data->rawdata_proc_entry);
        oem_data->rawdata_proc_entry = NULL;
    }

    if (oem_data->manual_proc_entry) {
        cts_info("  Remove manual proc entry");
        proc_remove(oem_data->manual_proc_entry);
        oem_data->manual_proc_entry = NULL;
    }

    if (oem_data->diffdata_proc_entry) {
        cts_info("  Remove diffdata proc entry");
        proc_remove(oem_data->diffdata_proc_entry);
        oem_data->diffdata_proc_entry = NULL;
    }
