...
static inline bool earjack_state_path_valid(const char *path)
{
    ...
    struct device_node *np)
{
    ...
    if (ed_data->enable && !earjack_state_path_valid(ed_data->earjack_state_filepath)) {
        cts_warn("Earjack detect enabled but state filepath is empty, disable it");
...
static int start_earjack_detect(struct cts_earjack_detect_data *ed_data)
{
    ...
    if (!earjack_state_path_valid(ed_data->earjack_state_filepath)) {
        cts_warn("Start detect with invalid filepath, disable earjack detect");
        ed_data->enable = false;
        return 0;
    }
    ...
    {
        struct file *file = filp_open(ed_data->earjack_state_filepath, O_RDONLY, 0);
        if (IS_ERR(file)) {
            int ret = PTR_ERR(file);
            if (ret == -ENOENT) {
                cts_warn("Earjack state file '%s' missing, disable detect",
                    ed_data->earjack_state_filepath);
                ed_data->enable = false;
                return 0;
            }
            cts_err("Open earjack state file '%s' failed %d(%s)",
...
static int get_earjack_state(struct cts_earjack_detect_data *ed_data)
{
    ...
    if (!earjack_state_path_valid(ed_data->earjack_state_filepath)) {
        cts_warn("Get state with invalid filepath");
        return -ENODEV;
    }
...
int cts_earjack_detect_init(struct chipone_ts_data *cts_data)
{
    ...
    cts_info("  Filepath: '%s'",
        earjack_state_path_valid(ed_data->earjack_state_filepath) ?
        ed_data->earjack_state_filepath : "(none)");
