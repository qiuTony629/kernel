        ...
        struct device_node *dev_node)
{
    ...
    /* Parse touch coordinate offset for multi-display setup */
    ret = of_property_read_u32(dev_node, "chipone,touch-offset-x", &pdata->offset_x);
    if (ret) {
        pdata->offset_x = 0;
        cts_info("  %-12s: %d (default)", "offset_x", pdata->offset_x);
    } else {
        cts_info("  %-12s: %d", "offset_x", pdata->offset_x);
    }

    ret = of_property_read_u32(dev_node, "chipone,touch-offset-y", &pdata->offset_y);
    if (ret) {
        pdata->offset_y = 0;
        cts_info("  %-12s: %d (default)", "offset_y", pdata->offset_y);
    } else {
        cts_info("  %-12s: %d", "offset_y", pdata->offset_y);
    }

    /* Parse panel notifier for multi-display setup */
    pdata->panel_node = of_parse_phandle(dev_node, "rockchip,panel-notifier", 0);
    if (pdata->panel_node) {
        pdata->panel_name = of_node_full_name(pdata->panel_node);
        cts_info("  %-12s: %s", "panel", pdata->panel_name ? pdata->panel_name : "unknown");
    } else {
        pdata->panel_node = NULL;
        pdata->panel_name = NULL;
        cts_info("  %-12s: (none)", "panel");
    }
...
#endif
{
    ...
    {
        char input_name[64];
        snprintf(input_name, sizeof(input_name), "%s-i2c%d",
            CFG_CTS_DEVICE_NAME, pdata->i2c_client->adapter->nr);
        input_dev->name = kstrdup(input_name, GFP_KERNEL);
        if (!input_dev->name) {
            cts_err("Failed to allocate input device name");
    ...
    /* For multi-display setup: if offset is set, extend range to include offset */
    input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
            pdata->res_y + (pdata->offset_x > 0 ? pdata->offset_x : 0), 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
            pdata->res_x + (pdata->offset_y > 0 ? pdata->offset_y : 0), 0, 0);
#else /* CFG_CTS_SWAP_XY */
    /* For multi-display setup: if offset is set, extend range to include offset */
    input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
            pdata->res_x + (pdata->offset_x > 0 ? pdata->offset_x : 0), 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
            pdata->res_y + (pdata->offset_y > 0 ? pdata->offset_y : 0), 0, 0);
        ...
        struct cts_device_touch_msg *msgs, int num)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        /* Note: Coordinate offset is only for multi-display setup where
         * displays are combined into a single logical screen.
         * For independent displays, offset should be 0 and user-space
         * should map input devices to displays via device name or sysfs.
         */
        if (pdata->offset_x != 0 || pdata->offset_y != 0) {
            cts_dbg("  Apply offset: x=%u y=%u offset_x=%u offset_y=%u",
                x, y, pdata->offset_x, pdata->offset_y);
            x += pdata->offset_x;
            y += pdata->offset_y;
        }
