...
int cts_tool_init(struct chipone_ts_data *cts_data)
{
    ...
    /*
     * When there are multiple identical devices (e.g. on i2c4 and i2c5),
     * using a fixed proc filename will cause duplicate registration errors.
     * Generate a unique name per device based on bus number and address.
     */
#ifdef CONFIG_CTS_I2C_HOST
    {
        char proc_name[32];
        ...
        cts_data->procfs_entry = proc_create_data(proc_name,
...
void cts_tool_deinit(struct chipone_ts_data *data)
{
    ...
    if (data->procfs_entry) {
        proc_remove(data->procfs_entry);
        data->procfs_entry = NULL;
    }
}
